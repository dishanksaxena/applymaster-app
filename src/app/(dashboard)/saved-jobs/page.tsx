    ...
    <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.08 } } }} className="space-y-6 max-w-[1200px] mx-auto">
      ...
      {loading ? (
        ...
        <motion.div variants={fadeUp} className="text-center py-20">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--text-faint)" strokeWidth="1.5" className="mx-auto mb-4"><path d="M19 21l-7-5-7 5V5a2 2 0 012-2h10a2 2 0 012 2z"/></svg>
          <p className="text-[15px] font-semibold text-[var(--text-faint)]">No saved jobs yet</p>
          <p className="text-[12px] text-[var(--text-faint)] mt-1">Start searching to find and save jobs</p>
          <Link href="/jobs" className="inline-block mt-4 px-6 py-3 rounded-xl text-[13px] font-bold text-[var(--text-on-accent)]" style={{ background: 'linear-gradient(135deg, var(--accent), var(--accent-solid))' }}>
            Browse Jobs →
          </Link>
        </motion.div>
      ) : (
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <h3 className="text-[15px] font-bold text-ink truncate">{job.title}</h3>
                  <p className="text-[13px] text-[var(--accent)] mt-0.5">{job.company}</p>
