    ...
    <motion.div initial="hidden" animate="show" variants={fadeUp} className="space-y-8 max-w-4xl mx-auto">
      ...
      <div className="text-center py-16">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }} className="inline-block mb-4">
          <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--text-faint)" strokeWidth="1.5"><path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 01-3.46 0" /></svg>
        </motion.div>
        <p className="text-[15px] font-semibold text-[var(--text-faint)]">No notifications yet</p>
        <p className="text-[12px] text-[var(--text-faint)] mt-1">We'll notify you when you get interview invites and job matches</p>
      </div>
    </motion.div>
  )
}
